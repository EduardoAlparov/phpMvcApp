...
class User
{
    ...
    public static function getByEmail(string $email)
    {
        $db = Db::getInstance();
        $data = $db->fetchOne(
            "SELECT * FROM blog_users WHERE email = :email",
            __METHOD__,
            [':email' => $email]
        );
        if (!$data) {
    ...
    public static function getByIds(array $userIds)
    {
        ...
        $data = $db->fetchAll(
            "SELECT * fROM blog_users WHERE id IN($idsString)",
            __METHOD__
        );
        if (!$data) {
            return [];
    ...
    {
        $db = Db::getInstance();
        $data = $db->fetchOne("SELECT * fROM blog_users WHERE id = :id", __METHOD__, [':id' => $id]);
        if (!$data) {
            return null;
        }
    ...
    public static function getList(int $limit = 10, int $offset = 0): array
    {
        $db = Db::getInstance();
        $data = $db->fetchAll(
            "SELECT * fROM blog_users LIMIT $limit OFFSET $offset",
            __METHOD__
        );
        if (!$data) {
            return [];
